...
export default function DashboardLayout({
  ...
  return (
    <div className="student-app-shell text-white">
      {/* Mobile Header */}
      <header className="student-header shrink-0 z-30 flex items-center justify-between border-b border-purple-500/30 bg-gradient-to-r from-[#1a1233] via-[#3b2a73] to-[#581c87] px-3">
        <button
          onClick={() => setSidebarOpen(true)}
          className="flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br from-pink-500 to-purple-500 text-white shadow-lg shadow-purple-500/40"
          aria-label="Open menu"
        >
          <Menu className="h-5 w-5" />
        </button>
        <span className="sr-only">MindSpark Student Hub</span>
      </header>
      ...
      <AnimatePresence>
        {sidebarOpen && (
          <motion.aside
            ...
            className="fixed bottom-0 left-0 top-0 z-50 w-[280px] border-r border-purple-500/30 bg-gradient-to-b from-[#1a1233] via-[#3b2a73] to-[#581c87] p-4 text-white"
          >
            ...
            <nav className="space-y-2">
              {navItems.map((item, idx) => {
                ...
                const navColors = [
                  'bg-gradient-to-br from-[#9333ea] to-[#c084fc] shadow-blue-500/40',
            ...
            {/* Stats */}
            <div className="mt-4 flex items-center justify-center gap-2 rounded-lg border border-purple-400/20 bg-purple-900/30 px-3 py-2">
              <MobileStats />
            </div>
